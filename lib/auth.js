...
function clientCredentialsFlow(options) {
    ...
    flow.authenticate = flow.emit.bind(flow, 'authenticate');
    flow.getToken = function () {
        return settings.accessToken;
    };
    flow.refreshToken = function () {
        ...
        .then(function (result) {
            ...
            flow.emit('accessToken', settings.accessToken, result.data.expires_in * 1000);
        });
    };
...
function refreshTokenFlow(options) {
    ...
    flow.refreshToken = function () {
        ...
        .then(function (result) {
            ...
            flow.emit('accessToken', settings.accessToken, result.data.expires_in * 1000);
            flow.emit('refreshToken', settings.refreshToken);
        });
    };
